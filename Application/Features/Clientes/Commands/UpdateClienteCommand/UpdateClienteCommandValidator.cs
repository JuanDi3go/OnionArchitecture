...
namespace Application.Features.Clientes.Commands.UpdateClienteCommand
{
    public class UpdateClienteCommandValidator:AbstractValidator<UpdateClienteCommand>
    {
        ...
        {
            RuleFor(p => p.Id).NotEmpty().WithMessage("El id no puede ser vacio");
            ...
            RuleFor(p => p.Apellido).NotEmpty().WithMessage("{PropertyName} no puede ser vacio")
                .MaximumLength(80).WithMessage("{PropertyName} no debe exceder de {MaxLenght} caracteres");
            RuleFor(p => p.FechaNacimiento).NotEmpty().WithMessage("Fecha de nacimiento no puede ser vacio");
            RuleFor(p => p.Telefono).NotEmpty().WithMessage("{PropertyName} no puede ser vacio")
            ...
            RuleFor(p => p.Direccion).NotEmpty().WithMessage("{PropertyName} no puede ser vacio")
